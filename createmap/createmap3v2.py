...
class PathManager:
    ...
    def createmap(self, path):
        ...
        self.add_Directory(self.child)
        self.wait()

    def work(self, obj):
        with self._directories_lock:
            local = self.directories
            local.append(obj)
            self.directories = local
        obj.scan()
    ...
    def add_Directory(self, obj):
        threading.Thread(target=self.work, args=(obj,)).start()
...
class Directory:
    ...
    def addFolder(self, item):
        ...
        self.manager.add_Directory(Directory(item, self.manager))
